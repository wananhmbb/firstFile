<template>
	<theme>
		<view class="all" slot="theme">
			<house-header :headTypeTwo="navtitle"></house-header>
			<view class="form">
				<u-form :model="form" ref="uForm" label-width="150" :error-type="errorType">
					<u-form-item label="预约项目"><u-input v-model="title" :disabled="true" :custom-style="customStyle" /></u-form-item>
					<u-form-item label="预约门店" prop="reservedStore">
						<u-input v-model="form.reservedStore" placeholder="请选择 >" :disabled="true" @click="chooseStore" :custom-style="customStyle" />
					</u-form-item>
					<view class="reservedTimeBox"></view>
					<u-form-item label="预计时间"><u-input v-model="estimateTime" :disabled="true" :custom-style="customStyle" /></u-form-item>
				</u-form>
				<scorllTime @getDisabled="getDisabled" :scorllTimeList="scorllTimeList" :estimateTime="estimateTime" @getparams="getparams"></scorllTime>
				<view class="mask"></view>
				<view class="btnBox">
					<view class="reservedTimeBox"></view>
					<view class="btn">
						<u-button v-if="!appointmentdisabled" type="primary" shape="circle" throttle-time="3000" @click="goAppointment()">确认预约</u-button>
						<u-button v-if="appointmentdisabled" type="primary" :plain="true" shape="circle" throttle-time="3000" @click="goNoAppointment()">确认预约</u-button>
					</view>
				</view>
				<!-- <u-select
          v-model="storeShow"
        ...
        ></u-select> -->
            ...
            ></u-tabs> -->
						<view class="listItem" v-for="(item, index) in TabList" :key="index" @click="tabOnChanged(index)">
							<view :class="tabBoxcurrent == index ? `item-name active` : 'item-name'">{{ item.name }}</view>
							<view v-show="tabBoxcurrent == index" class="line"></view>
						</view>
					</view>
					<view class="storeBox" v-if="storeList.length > 0">
						<view class="storeBoxView" v-for="(item, index) in storeList" :key="index" @click="check(index)">
							<view class="storeBoxView-one">
								<view :class="indexIn === index ? 'storeBoxView-first ' : 'storeBoxView-first1'">
							...
							<view class="storeBoxView-two">
								...
								<view v-if="item.phone">
									电话：
									<text class="phone" v-for="(phoneItem, phoneIndex) in item.phoneNumber" @click="phoneCall(phoneItem)" :key="phoneIndex">{{ phoneItem }}</text>
								</view>
							</view>
						</view>
					</view>
			...
			<!-- <view v-if="empty" class="empty"><x-icon iconName="service-no-data"></x-icon></view> -->
		</view>
	</theme>
</template>

<script>
import { getReservationStore, getDoctorStore, getReservationTime, makeAppointment } from '@/api/service';
import scorllTime from '@/other-pages/pages/examinationAppointment/components/scorllTime';
export default {
	components: {
		scorllTime
	},
	data() {
		return {
			...
			form: {
				reservedStore:''
					// {
					// address: "四川省成都市成华区航天路64号御莎 大厦3楼"
					// ancestors: "0"
					// delFlag: "0"
					// deptId: "1625701415511068674"
					// deptName: "中核康宁健康管理中心"
					// email: null
					// latitude: null
					// leader: null
	...
	watch: {},
	onLoad() {
		this.navtitle = this.$Route.query.navname;
		this.medicalCardId = '1625446713678565377';
		this.servicePackageUserId = '1626167396712779778';
		this.estimateTime = this.getNowTime();
		this.storeId = '1625701415511068674';
					...
					if (res.code === 200) {
						...
						if (this.TabList.length > 0) {
							...
							if (envVersion === 'trial' || envVersion === 'develop') {
								...
								this.storeList = storeList.filter(item => !item.deptName.includes('模拟'));
							}
